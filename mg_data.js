...
function MG_getEmployeeEmails() {
  try {
    const cached = MG_getCached();
    if (cached) return cached.employees || [];
    return [];
  } catch(e) { return []; }
}
function MG_isAdmin(email) {
  if (email === MG_ADMIN_EMAIL) return true;
  // Vérifier aussi les admins dynamiques depuis le cache
  try {
    const cached = MG_getCached();
    if (cached && cached.admins) return cached.admins.includes(email);
  } catch(e) {}
  return false;
}
async function MG_isAdminAsync(email) {
  if (email === MG_ADMIN_EMAIL) return true;
  const db = await MG_loadDB();
  return (db.admins||[]).includes(email);
}
function MG_isSuperAdmin(email) { return email === MG_ADMIN_EMAIL; }
async function MG_isEmployee(email) {
  if (MG_isAdmin(email)) return true;
  const db = await MG_loadDB();
  return (db.employees||[]).includes(email);
}
...
async function MG_loadDB() {
  ...
  if (!MG_BIN_ID || !MG_BIN_KEY) {
    try {
      ...
      if (!raw) { const db={users:{},employees:[],admins:[],invoices:[]}; return db; }
...
async function MG_upsertUser(uid, data) {
  const db = await MG_loadDB();
  let role = 'client';
  if (data.email === MG_ADMIN_EMAIL || (db.admins||[]).includes(data.email)) {
    role = 'admin';
  ...
  if (!db.users[uid]) {
    ...
    // Le nom Google n'est PAS stocké — seuls email et pseudo sont utilisés
    db.users[uid] = { uid, email:data.email, pseudo:'',
      role, personnages:[], createdAt:MG_today() };
  } else {
    // Reconnexion — on met à jour email/role, le pseudo est conservé tel quel
...
async function MG_removePoints(uid, pid, amount) {
  const db = await MG_loadDB();
  if (!db.users[uid]) return null;
  const p = db.users[uid].personnages.find(x => x.id === pid);
  if (!p) return null;
...
async function MG_addAdmin(email) {
  const db = await MG_loadDB();
  if (!db.admins) db.admins = [];
  if (!db.admins.includes(email)) db.admins.push(email);
  // Mettre à jour le rôle du user si existant
  Object.values(db.users).forEach(u => { if (u.email === email) u.role = 'admin'; });
  await MG_saveDB(db);
}
async function MG_removeAdmin(email) {
  if (email === MG_ADMIN_EMAIL) return; // impossible de retirer le super-admin
  const db = await MG_loadDB();
  db.admins = (db.admins||[]).filter(e => e !== email);
  Object.values(db.users).forEach(u => { if (u.email === email) u.role = 'client'; });
  await MG_saveDB(db);
}
async function MG_getAdminEmails() {
  const db = await MG_loadDB();
  return [MG_ADMIN_EMAIL, ...(db.admins||[]).filter(e => e !== MG_ADMIN_EMAIL)];
}
...
async function MG_clearInvoices() {
  const db = await MG_loadDB();
  db.invoices = [];
  await MG_saveDB(db);
}
