...
const MG_CATALOGUE = [
  ...
  { name:'MP7',            cat:'smg',       mode:'Auto',     cal:'4.6×30mm',   mag:40,  price:12000,  marge:10, spec:false },
  ...
  { name:'M500',           cat:'pompe',     mode:'Pompe',    cal:'12 Gauge',   mag:5,   price:17500, marge:10, spec:true  },
  ...
  // Explosifs
  { name:'C4',                       cat:'explosif',   mode:'—', cal:'—',            mag:null, price:0,    marge:0, spec:true  },
  { name:'Détonateur',               cat:'explosif',   mode:'—', cal:'—',            mag:null, price:0,    marge:0, spec:true  },
  // Explosifs
  { name:'C4',                       cat:'explosif',   mode:'—', cal:'—', mag:null, price:0,    marge:0, spec:true  },
  { name:'Détonateur',               cat:'explosif',   mode:'—', cal:'—', mag:null, price:0,    marge:0, spec:true  },
  // Équipement
...
async function MG_getPerksOverrides() {
  if (!MG_isFirebaseConfigured()) {
    try { return JSON.parse(localStorage.getItem('mg_perks_v1')||'{}'); } catch(e) { return {}; }
  }
  try {
    const db  = await MG_getDB();
...
async function MG_getActivePerks() {
  ...
  const result = {
    tiers:     JSON.parse(JSON.stringify(MG_TIERS)),
    contracts: JSON.parse(JSON.stringify(MG_CONTRACTS))
  };
  if (ov.tiers) {
    ov.tiers.forEach((t,i) => { if (result.tiers[i]) result.tiers[i].perks = t; });
  }
  if (ov.contracts) {
    Object.keys(ov.contracts).forEach(k => {
      if (result.contracts[k]) result.contracts[k].perks = ov.contracts[k];
    });
  }
  return result;
}
...
// ═══════════════════════════════════════════════════
//  AVANTAGES ÉDITABLES (Firestore override)
// ═══════════════════════════════════════════════════
async function MG_getPerksOverrides() {
  try {
    if (!MG_isFirebaseConfigured())
      return JSON.parse(localStorage.getItem('mg_perks_v1')||'null');
    const db  = await MG_getDB();
    const doc = await db.collection('meta').doc('perks').get();
    return doc.exists ? doc.data() : null;
  } catch(e) { return null; }
...
async function MG_savePerksOverrides(data) {
  ...
  const db = await MG_getDB();
  await db.collection('meta').doc('perks').set(data, { merge:true });
}

// Applique les overrides aux données par défaut
async function MG_getActivePerks() {
  const ov = await MG_getPerksOverrides();
  const tiers     = JSON.parse(JSON.stringify(MG_TIERS));
  const contracts = JSON.parse(JSON.stringify(MG_CONTRACTS));
  if (!ov) return { tiers, contracts };
  if (ov.tiers)     ov.tiers.forEach((perks, i) => { if (tiers[i]) tiers[i].perks = perks; });
  if (ov.contracts) Object.keys(ov.contracts).forEach(k => { if (contracts[k]) contracts[k].perks = ov.contracts[k]; });
